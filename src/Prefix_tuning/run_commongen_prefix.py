...
from modeling_bart import NoPosMyBartForConditionalGeneration
from modeling_gpt2 import MyGPT2LMHeadModel
from modeling_prefix import PrefixModelBart, PrefixModelGPT2
from Finetune.run_fine_tuning import set_seed, count_parameters
logger = logging.getLogger(__name__)

MODEL_CLASSES = {
    'gpt2': (GPT2Config, MyGPT2LMHeadModel, GPT2Tokenizer),
    'bart': (BartConfig, NoPosMyBartForConditionalGeneration, BartTokenizer),
}
...
def load_and_cache_examples(args, tokenizer, evaluate=False, max_q = None, max_a = None):
    ...
    dataset = CommonGenDataset(tokenizer, data_dir=args.data_dir, type_path=type_path, max_source_length=args.max_src_len, max_target_length=args.max_tgt_len)
    return dataset

def model_step(batch: dict, model, core_model, tokenizer):
    source_ids, source_mask, target_ids = batch["input_ids"], batch["attention_mask"], batch["decoder_input_ids"]
    if isinstance(core_model, NoPosMyBartForConditionalGeneration):
...
def evaluate(args, model, core_model, tokenizer, prefix="", max_q = None, max_a = None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            all_prefix = model.get_prefix(batch['input_ids'])
            generated_ids = core_model.generate(input_ids=batch['input_ids'], attention_mask=batch['attention_mask'], prefix_key_values=all_prefix, use_cache=True, num_beams=5, 
				length_penalty=0.6, max_length=128, repetition_penalty=2.0)    # DIFF HERE
        nb_eval_steps += 1
    ...
    with open(os.path.join(eval_output_dir, prefix, "eval_generation.txt"), "w") as f:
...
def main():
    ...
    if args.do_train:
        ...
        os.system("cp run_commongen_prefix.py %s" % os.path.join(args.output_dir, 'run_commongen_prefix.py'))
