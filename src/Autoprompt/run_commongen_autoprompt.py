...
import sys
sys.path.append('../')
from run_autoprompt import set_seed, add_task_specific_tokens, load_pretrained, hotflip_attack

from utils import CommonGenDataset
...
def run_model(args):
    ...
    train_dataset = CommonGenDataset(tokenizer, data_dir=args.data_dir, type_path=type_path, prefix=args.template)
    train_loader = DataLoader(train_dataset, batch_size=args.bsz, shuffle=True, collate_fn=train_dataset.collate_fn)
    
    dev_dataset = CommonGenDataset(tokenizer, data_dir=args.data_dir, type_path='dev', prefix=args.template)
...
def run_eval(args):
    ...
    dev_dataset = CommonGenDataset(tokenizer, data_dir=args.data_dir, type_path='dev', prefix=args.template)
    dev_loader = DataLoader(dev_dataset, batch_size=args.eval_size, shuffle=False, collate_fn=dev_dataset.collate_fn)
    with open(os.path.join(args.outdir, 'trigger_ids_checkpoints.json'), 'r') as fin:
        best_trigger_ids = json.load(fin)
    trigger_ids = torch.tensor(best_trigger_ids[0][0], dtype=torch.long, device=device)
    logger.info('Evaluating')
    preds = []
    for model_inputs in tqdm(dev_loader):
        for k, v in model_inputs.items():
        ...
        with torch.no_grad():
            trigger_mask = model_inputs.pop('trigger_mask')
            tmp_trigger_ids = trigger_ids.repeat(trigger_mask.size(0), 1)
            model_inputs['input_ids'] = model_inputs['input_ids'].masked_scatter(trigger_mask, tmp_trigger_ids)
            generated_ids = model.generate(input_ids=model_inputs['input_ids'], attention_mask=model_inputs['attention_mask'], use_cache=True, num_beams=5, 
				length_penalty=0.6, max_length=128, repetition_penalty=2.0, min_length=len(model_inputs['decoder_input_ids'][0])+3)
            gen_text = tokenizer.batch_decode(generated_ids[:, args.num_triggers+1:], skip_special_tokens=True, clean_up_tokenization_spaces=True)
            gen_text = [t if t.endswith('.') else t+'.' for t in gen_text]
            preds += gen_text
    with open(os.path.join(args.outdir, "eval_generation.txt"), "w") as f:
...
if __name__ == '__main__':
    ...
    if args.do_train:
        ...
        os.system("cp run_commongen_autoprompt.py %s" % os.path.join(args.outdir, 'run_commongen_autoprompt.py'))
